...
export default function ProjectPage() {
  ...
  useEffect(() => { if (!accessToken || !documents.some((document) => ["queued", "indexing"].includes(document.status))) return; const timer = window.setInterval(() => void listDocuments(accessToken, id).then(setDocuments), 2000); return () => window.clearInterval(timer); }, [accessToken, documents, id]);
  ...
  if (loading || !user || !project || !accessToken) return <main className="center-state">Opening your project…{error && ` ${error}`}</main>;
