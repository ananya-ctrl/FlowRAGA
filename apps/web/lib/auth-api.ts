...
export type PipelineNodeType = "source" | "chunk" | "embed" | "retrieve" | "rerank" | "generate" | "evaluate";
export interface PipelineNode { id: string; type: PipelineNodeType; label: string; x: number; y: number; }
export interface PipelineEdge { id: string; source: string; target: string; }
export interface PipelineConfiguration { retrieval_mode: "hybrid" | "vector" | "keyword"; top_k: number; similarity_threshold: number; rerank: boolean; chunk_size_words: number; chunk_overlap_words: number; generation_model: string; nodes: PipelineNode[]; edges: PipelineEdge[]; }
...
export interface PipelineTemplate { name: string; configuration: PipelineConfiguration; }
...
export function listPipelineTemplates(accessToken: string, projectId: string) { return request<PipelineTemplate[]>(`/api/v1/projects/${projectId}/pipelines/templates`, {headers: {Authorization: `Bearer ${accessToken}`}}); }
...
export function updatePipeline(accessToken: string, projectId: string, pipelineId: string, input: {name: string; configuration: PipelineConfiguration}) { return request<Pipeline>(`/api/v1/projects/${projectId}/pipelines/${pipelineId}`, {method: "PUT", headers: {Authorization: `Bearer ${accessToken}`}, body: JSON.stringify(input)}); }
